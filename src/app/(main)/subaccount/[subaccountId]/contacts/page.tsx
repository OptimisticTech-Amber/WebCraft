...
import CreateContactButton from './_component/CreateContactButton'
...
const ContactPage= async ({params}: Props) => {
    ...
    const formalTotal = (ticket:Ticket[]) =>{
  if(
...
const laneAmt = ticket.reduce((sum,ticket)=> sum + (Number(ticket?.value) || 0),0)
            ...
            allContacts.map((contact) => {
              return (
                <TableRow key={contact.id}>
                  ...
                  <TableCell>
                    {formalTotal(contact.Ticket) === null ? (
                  ...
                  <TableCell>{formalTotal(contact.Ticket)}</TableCell>
                </TableRow>
              );
            })}
        </TableBody>
